...
import type { WorkOrder, WorkOrderDetail, TaskPriority } from '@/types'
...
const editingDetail = ref<WorkOrderDetail | null>(null)
...
async function loadWorkOrders() {
  await workOrderStore.fetchWorkOrders({
    ...
    name: workOrderName.value || undefined
  })
}
...
async function handleAddWorkOrder() {
  await workOrderStore.createWorkOrder({
    ...
    register_time: newWorkOrder.value.register_time.replace('T', ' '),
    publisher: newWorkOrder.value.publisher,
    remark: newWorkOrder.value.remark,
    priority: newWorkOrder.value.priority
  })
  showAddDialog.value = false
  newWorkOrder.value = { name: '', register_time: new Date().toISOString().slice(0, 16), publisher: authStore.user?.username || '', remark: '', priority: 'normal' as TaskPriority }
  loadWorkOrders()
}

async function handleComplete(workOrder: WorkOrder) {
  await workOrderStore.completeWorkOrder(workOrder.id)
  loadWorkOrders()
}

async function handleDefer(workOrder: WorkOrder) {
  await workOrderStore.deferWorkOrder(workOrder.id)
  loadWorkOrders()
}
...
async function openDetail(workOrder: WorkOrder) {
  ...
  await workOrderStore.fetchWorkOrderDetails(workOrder.id)
}

function startEditPriority() {
  if (selectedWorkOrder.value) {
    editingPriorityValue.value = selectedWorkOrder.value.priority
    editingTaskPriority.value = true
  }
}

async function saveWorkOrderPriority() {
  if (!selectedWorkOrder.value) return
  await workOrderStore.updateWorkOrder(selectedWorkOrder.value.id, {
    priority: editingPriorityValue.value
  })
  selectedWorkOrder.value.priority = editingPriorityValue.value
  editingTaskPriority.value = false
  loadWorkOrders()
}
...
function openEditDetail(detail: WorkOrderDetail) {
...
async function handleSaveDetail() {
  ...
  if (editingDetail.value) {
    await workOrderStore.updateWorkOrderDetail(selectedWorkOrder.value.id, editingDetail.value.id, {
    ...
    await workOrderStore.createWorkOrderDetail(selectedWorkOrder.value.id, {
      name: newDetail.value.name,
      progress: newDetail.value.progress,
      time: newDetail.value.time.replace('T', ' '),
      remark: newDetail.value.remark
  ...
  await workOrderStore.fetchWorkOrderDetails(selectedWorkOrder.value.id)
}

async function handleDeleteDetail(detailId: number) {
  if (!selectedWorkOrder.value) return
  if (confirm('确定要删除这条明细吗？')) {
    await workOrderStore.deleteWorkOrderDetail(selectedWorkOrder.value.id, detailId)
    await workOrderStore.fetchWorkOrderDetails(selectedWorkOrder.value.id)
  }
}
    ...
    <div class="flex-1 flex flex-col">
      ...
      <header class="h-16 border-b glass flex items-center justify-between px-6">
        <div class="flex items-center gap-4">
          <h1 class="text-xl font-semibold">工单管理</h1>
          ...
          <CardContent class="p-6">
            <Tabs :model-value="activeTab" @update:model-value="onTabChange">
              <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-3">
                  ...
                  <Badge variant="outline" class="text-xs">
                    {{ workOrderStore.workOrders.length }} 条记录
                  </Badge>
                </div>
                  ...
                  <DialogContent glass class="max-w-4xl max-h-[99vh] min-h-[70vh]">
                    <DialogHeader class="flex-shrink-0 pb-4 border-b border-border/50">
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          <DialogTitle class="text-xl font-semibold">新增工单</DialogTitle>
                          <p class="text-sm text-muted-foreground mt-0.5">创建新的工单</p>
                        </div>
                      </div>
                    </DialogHeader>
                    <div class="flex-1 min-h-0 overflow-y-auto py-5">
                      <form @submit.prevent="handleAddWorkOrder" class="space-y-5">
                        <div class="bg-gradient-to-r from-slate-50/80 to-slate-100/50 dark:from-slate-800/50 dark:to-slate-800/30 rounded-2xl p-5 border border-slate-200/50 dark:border-slate-700/50 space-y-4">
                          <FormField name="name">
                            <FormItem>
                              ...
                              <FormControl>
                                <Input v-model="newWorkOrder.name" placeholder="请输入工单名称" required class="h-11 bg-white/80 dark:bg-slate-900/80 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                              </FormControl>
                            </FormItem>
                          </FormField>
                          <FormField name="priority">
                            <FormItem>
                              ...
                              <FormControl>
                                <select v-model="newWorkOrder.priority" class="h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 px-3 w-full focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                          ...
                          <FormField name="register_time">
                            <FormItem>
                              ...
                              <FormControl>
                                <Input v-model="newWorkOrder.register_time" type="datetime-local" required class="h-11 bg-white/80 dark:bg-slate-900/80 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500" />
                              </FormControl>
                            </FormItem>
                          </FormField>
                          <FormField name="publisher">
                            <FormItem>
                              ...
                              <FormControl>
                                <Input v-model="newWorkOrder.publisher" required class="h-11 bg-white/80 dark:bg-slate-900/80 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-slate-500/20 focus:border-slate-500" />
                              </FormControl>
                            </FormItem>
                          </FormField>
                          <FormField name="remark">
                            <FormItem>
                              ...
                              <FormControl>
                                <Input v-model="newWorkOrder.remark" placeholder="可选" class="h-11 bg-white/80 dark:bg-slate-900/80 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-slate-500/20 focus:border-slate-500" />
                              </FormControl>
                            </FormItem>
                          </FormField>
                        </div>
                  ...
                  <DialogContent glass class="max-w-4xl max-h-[99vh] min-h-[70vh]">
                    <DialogHeader class="flex-shrink-0 pb-4 border-b border-border/50">
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          <DialogTitle class="text-xl font-semibold">工单详情</DialogTitle>
                          <p class="text-sm text-muted-foreground mt-0.5">{{ selectedWorkOrder?.name }}</p>
                        </div>
                      </div>
                    </DialogHeader>
                      ...
                      <div class="bg-gradient-to-r from-slate-50/80 to-slate-100/50 dark:from-slate-800/50 dark:to-slate-800/30 rounded-2xl p-5 border border-slate-200/50 dark:border-slate-700/50">
                        ...
                        <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
                          <div class="flex items-center justify-between py-2 px-4 bg-white/60 dark:bg-slate-900/60 rounded-xl">
                            ...
                            <span class="font-medium">{{ selectedWorkOrder?.register_time ? new Date(selectedWorkOrder.register_time).toLocaleString('zh-CN') : '-' }}</span>
                          </div>
                          <div class="flex items-center justify-between py-2 px-4 bg-white/60 dark:bg-slate-900/60 rounded-xl">
                            <span class="text-muted-foreground flex items-center gap-2">
                              <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                            ...
                            <span class="font-medium">{{ selectedWorkOrder?.publisher }}</span>
                          </div>
                          <div class="flex items-center justify-between py-2 px-4 bg-white/60 dark:bg-slate-900/60 rounded-xl">
                            <span class="text-muted-foreground flex items-center gap-2">
                            ...
                            <Badge :variant="selectedWorkOrder?.is_completed ? 'default' : 'secondary'" :class="selectedWorkOrder?.is_completed ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-400'">
                              {{ selectedWorkOrder?.is_completed ? '已完成' : '待处理' }}
                            </Badge>
                          </div>
                          <div class="flex items-center justify-between py-2 px-4 bg-white/60 dark:bg-slate-900/60 rounded-xl">
                            <span class="text-muted-foreground flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-2">
                              <template v-if="!editingTaskPriority">
                                <Badge :class="`badge-${selectedWorkOrder?.priority}`">
                                  {{ selectedWorkOrder?.priority === 'urgent' ? '紧急' : selectedWorkOrder?.priority === 'medium' ? '中' : '正常' }}
                                </Badge>
                              ...
                              <template v-else>
                                ...
                                <Button variant="default" size="sm" class="h-7 px-3 text-xs bg-blue-600 hover:bg-blue-700" @click="saveWorkOrderPriority">保存</Button>
                                <Button variant="ghost" size="sm" class="h-7 px-3 text-xs" @click="cancelEditPriority">取消</Button>
                              </template>
                            </div>
                          </div>
                          <div v-if="selectedWorkOrder?.complete_time" class="flex items-center justify-between py-2 px-4 bg-white/60 dark:bg-slate-900/60 rounded-xl">
                            <span class="text-muted-foreground flex items-center gap-2">
                            ...
                            <span class="font-medium text-emerald-600 dark:text-emerald-400">{{ new Date(selectedWorkOrder.complete_time).toLocaleString('zh-CN') }}</span>
                          </div>
                        </div>
                        <div v-if="selectedWorkOrder?.remark" class="mt-4 p-4 bg-white/60 dark:bg-slate-900/60 rounded-xl">
                          <span class="text-muted-foreground text-sm flex items-center gap-2 mb-2">
                            <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"></path></svg>
                            备注
                          </span>
                          <span class="text-sm font-medium">{{ selectedWorkOrder.remark }}</span>
                        </div>
                      </div>
                      ...
                      <div class="border border-slate-200/50 dark:border-slate-700/50 rounded-2xl p-5">
                        <div class="flex items-center justify-between mb-4">
                          <h3 class="font-medium flex items-center gap-2">
                            ...
                            工单明细
                            <span class="text-xs text-muted-foreground font-normal">({{ workOrderStore.workOrderDetails.length }})</span>
                          </h3>
                        ...
                        <div v-if="workOrderStore.workOrderDetails.length === 0" class="text-center py-12">
                    ...
                    <DialogHeader class="flex-shrink-0 pb-4 border-b border-border/50">
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p class="text-sm text-muted-foreground mt-0.5">{{ editingDetail ? '修改工单明细信息' : '创建新的工单明细' }}</p>
                        </div>
                      </div>
                    </DialogHeader>
                    <div class="flex-1 min-h-0 overflow-y-auto py-5">
              ...
              <div class="flex items-center gap-4 mb-6 p-4 glass-card rounded-lg">
                <div class="flex items-center gap-2">
                  <label class="text-sm font-medium">工单名称</label>
                  <Input v-model="workOrderName" placeholder="请输入工单名称" class="w-40" />
                </div>
                <div class="flex items-center gap-2">
              ...
              <TabsContent value="pending">
                <WorkOrderTable :work-orders="workOrderStore.workOrders" @complete="handleComplete" @defer="handleDefer" @delete="handleDelete" @detail="openDetail" />
              </TabsContent>
              <TabsContent value="completed">
                <WorkOrderTable :work-orders="workOrderStore.workOrders" @detail="openDetail" />
              </TabsContent>
            </Tabs>
          </CardContent>
        </Card>
